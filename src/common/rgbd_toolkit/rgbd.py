...
def process_pcd(norm_pcd_time, process_index=0):
    ...
    for pcd in tqdm(norm_pcd_time, position=process_index):
            ...
            logging.info("timestamp of pcd: " + "{0:.2f}".format(round(pcd, 2))
                         + " with index " + str(i) + " path: " + str(pcd_path[i]))
            logging.info("timestamp of jpg: " +
    "{0:.2f}".format(round(norm_rgb_time[nn], 2))
    + " with index "
    + str(nn)
    + " path: "
     + str(rgb_path[nn]))
