...
#changed config file
res_blocks = [int(x) for x in CONFIG.RES_BLOCKS[0].split(",")]
logging.info('res_blocks: %s', res_blocks)
dropouts = [float(x) for x in CONFIG.DROPOUTS_LIST[0].split(",")]
...
model.fit(
    ...
    verbose=2 #diese Zeile wird ausgespuckt im Log
)
